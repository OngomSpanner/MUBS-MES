...
interface ProcessAssignment {
  ...
  created_at: string;
  /** Process task title (DB still returns step_name) */
  task_name: string;
  /** Task order within the process (DB still returns step_order) */
  task_order: number;
  standard_id: number;
...
export default function StaffProcessSteps() {
  ...
  const fetchAssignments = async () => {
    try {
      ...
      // Map legacy field names to task terminology (no backend change required).
      const mapped = raw.map((r: any) => ({
        ...r,
        task_name: String(r.task_name ?? r.step_name ?? '').trim(),
        task_order: Number(r.task_order ?? r.step_order ?? 0),
      }));
      ...
      <div className="table-card p-0 overflow-hidden" style={{ borderRadius: '20px', border: '1px solid #e2e8f0' }}>
        ...
        {assignments.length === 0 ? (
          <div className="text-center py-5 text-muted">
            ...
            No processes have been assigned to you yet.
          </div>
        ) : (
          <div className="table-responsive">
            <table className="table mb-0 align-middle">
              <thead className="bg-light">
                <tr>
                  <th className="ps-4">Process task</th>
                  <th>Activity</th>
                  <th>Standard</th>
                  <th>Status</th>
              ...
              <tbody>
                {assignments.map(a => {
                  ...
                  return (
                    <tr key={a.id}>
                      <td className="ps-4">
                        <div className="fw-bold text-dark" style={{ fontSize: '.88rem' }}>{a.task_name}</div>
                        <div className="text-muted" style={{ fontSize: '.72rem' }}>Task #{a.task_order + 1}</div>
                      </td>
                      <td>
            ...
            <div className="modal-content border-0 shadow-lg" style={{ borderRadius: '14px' }}>
              ...
              <div className="modal-body px-4 py-3">
                ...
                <div className="p-3 rounded mb-3" style={{ background: '#f0f9ff', border: '1px solid #bae6fd', fontSize: '0.82rem' }}>
                  <div className="fw-bold text-dark">{selected.task_name}</div>

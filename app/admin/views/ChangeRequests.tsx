...
export default function ChangeRequestsView() {
  ...
  const loadRequests = useCallback(async () => {
    ...
    try {
      const url = statusFilter
        ? `/api/admin/change-requests?status=${encodeURIComponent(statusFilter)}`
  ...
  const closeReview = useCallback(() => {
    ...
    if (searchParams.get('id')) {
      router.replace('/admin?pg=change-requests');
    }
  ...
  useEffect(() => {
    ...
    axios
      .get(`/api/admin/change-requests/${deepLinkId}`)
      .then((res) => {
  ...
  const handleSaveReview = async (e: FormEvent) => {
    ...
    setReviewError(null);
    try {
      const res = await axios.patch(`/api/admin/change-requests/${selected.id}`, {
        status: reviewStatus,
        adminNotes,
      });

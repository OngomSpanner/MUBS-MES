...
export async function generateStaffWorkforceAssessmentsReport(): Promise<StaffWorkforceAssessmentsReport> {
  const window = getRollingReportFyWindow();
  const yearKeys = window.map((y) => y.key);
  const years = labelsFromFyWindow(window);
  const byDetail = await loadAssessmentCounts(yearKeys);

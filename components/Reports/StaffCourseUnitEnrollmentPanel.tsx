...
export default function StaffCourseUnitEnrollmentPanel({ facultyFilter: externalFaculty, hideFacultyFilter }: Props = {}) {
  ...
  const buildExportRows = (r: StaffCourseUnitEnrollmentReport) => {
    const body = r.rows.map((row) => [row.facultyName, row.courseUnitName, formatCount(row.studentCount)]);
  ...
  const exportPDF = async () => {
    ...
    autoTable(doc, {
      startY: metaY + 4,
      head: [['Faculty / school', 'Course unit', 'No of students']],
  ...
  const exportExcel = () => {
    ...
    const ws = XLSX.utils.aoa_to_sheet([
      ...
      [],
      ['Faculty / school', 'Course unit', 'No of students'],
        ...
        <table className="table mb-0 staff-course-unit-enrollment-table">
          <thead>
            <tr>
              ...
              <th className="text-start" style={{ minWidth: '260px' }}>
                Course unit
              </th>
          ...
          <tbody>
            {loading ? (
              ...
              <>
                {report.rows.map((row) => (
                  <tr key={`${row.facultyName}-${row.courseUnitName}`}>

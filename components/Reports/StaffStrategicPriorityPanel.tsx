...
import type { StaffStrategicPriorityReport } from '@/lib/hrms/staff-strategic-priority';
...
export default function StaffStrategicPriorityPanel() {
  const [report, setReport] = useState<StaffStrategicPriorityReport | null>(null);
  ...
  const fetchReport = useCallback(async () => {
    ...
    setError(null);
    try {
      const params = new URLSearchParams({ type: 'staff-strategic-priority' });
      ...
      setReport(data.data as StaffStrategicPriorityReport);
    } catch (e) {
      console.error('staff-strategic-priority error', e);
      setError('Could not load strategic priority staff data.');
      setReport(null);
    } finally {
      setLoading(false);
    }
  ...
  const buildTableBody = (r: StaffStrategicPriorityReport) =>
    r.rows.map((row) => [row.pillarLabel, ...r.yearKeys.map((key) => formatPct(row.byYear[key]))]);
  ...
  const exportPDF = async () => {
    ...
    doc.setFontSize(14);
    doc.text('% of staff in strategic priority areas', 14, 15);
    doc.setFontSize(9);
    let metaY = 22;
    ...
    autoTable(doc, {
      startY: metaY + 4,
      head: [['Strategic priority area', ...report.yearKeys.map((key) => report.years[key] ?? key)]],
      body: buildTableBody(report),
      styles: { fontSize: 7, cellPadding: 2 },
      headStyles: { fillColor: [30, 92, 164], textColor: 255, fontStyle: 'bold' },
    });

    doc.save('Staff_Strategic_Priority.pdf');
  };
  ...
  const exportExcel = () => {
    ...
    const ws = XLSX.utils.aoa_to_sheet([
      ['% of staff in strategic priority areas'],
      ['Faculty Name', report.facultyName],
      ['Department/Unit Name', report.departmentName],
      ['Number of Staff', report.numberOfStaff],
      [],
      ['Strategic priority area', ...report.yearKeys.map((key) => report.years[key] ?? key)],
      ...buildTableBody(report),
    ]);
    const wb = XLSX.utils.book_new();
    XLSX.utils.book_append_sheet(wb, ws, 'Strategic Priority');
    XLSX.writeFile(wb, 'Staff_Strategic_Priority.xlsx');
  };
  ...
  return (
    <div className="table-card">
      ...
      <div className="table-responsive">
        <table className="table mb-0 staff-strategic-priority-table">
          <thead>
            <tr>

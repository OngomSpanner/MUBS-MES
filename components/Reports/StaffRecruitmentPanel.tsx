...
const REPORT_TITLE = 'Number of staff recruited in the past five years';

function yearBandClass(index: number): string {
  return index % 2 === 0 ? 'establishment-year-band-prev' : 'establishment-year-band-current';
}

function yearCellClass(index: number): string {
...
export default function StaffRecruitmentPanel() {
  ...
  const fetchReport = useCallback(async () => {
    ...
    try {
      ...
      setReport(data.data as StaffRecruitmentReport);
    } catch (e) {
      console.error('staff-recruitment error', e);
      setError('Could not load staff recruitment data.');
      setReport(null);
    } finally {
      setLoading(false);
    }
  ...
  const buildTableBody = (r: StaffRecruitmentReport) => {
  ...
  const exportPDF = async () => {
    ...
    metaY += 6;
    doc.text(`Staff recruited (in filter): ${report.numberOfStaff}`, 14, metaY);
    metaY += 6;
    if (pwdFilter !== 'all') {
      const pwdLabel =
        pwdFilter === 'yes'
    ...
    doc.save('Staff_Recruitment.pdf');
  };
  ...
  const exportExcel = () => {
    ...
    const ws = XLSX.utils.aoa_to_sheet([
      ...
      ['Staff recruited (in filter)', report.numberOfStaff],
      [],
      headerRow1,
      ...body,
    ]);
    const wb = XLSX.utils.book_new();
    XLSX.utils.book_append_sheet(wb, ws, 'Staff Recruitment');
    XLSX.writeFile(wb, 'Staff_Recruitment.xlsx');
  };
      ...
      {report && !loading && !error && (
        <div className="px-4 py-2 border-bottom bg-light small">
          ...
          <div className="d-flex flex-wrap gap-3">
            ...
            <span>
              <span className="text-muted">Staff recruited:</span>{' '}
              <strong>{report.numberOfStaff.toLocaleString()}</strong>
            </span>
            <span className="text-muted">
              Years:{' '}
              {report.yearKeys.map((k) => report.years[k]).join(' · ')}
            </span>
          </div>
        </div>
      )}

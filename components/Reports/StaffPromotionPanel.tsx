...
import type { StaffPromotionReport } from '@/lib/hrms/staff-promotion';

const GENDER_METRICS = ['male', 'female', 'pwd'] as const;
const GENDER_HEADERS = ['Male', 'Female', 'PWD'] as const;

export default function StaffPromotionPanel() {
  const [report, setReport] = useState<StaffPromotionReport | null>(null);
  ...
  const fetchReport = useCallback(async () => {
    ...
    setError(null);
    try {
      const params = new URLSearchParams({ type: 'staff-promotion' });
      ...
      setReport(data.data as StaffPromotionReport);
    } catch (e) {
      console.error('staff-promotion error', e);
      setError('Could not load staff promotion data.');
      setReport(null);
    } finally {
      setLoading(false);
    }
  ...
  const buildTableBody = (r: StaffPromotionReport) => {
  ...
  const exportPDF = async () => {
    ...
    doc.setFontSize(14);
    doc.text('Staff Promotions', 14, 15);
    doc.setFontSize(9);
    let metaY = 22;
    ...
    metaY += 6;
    doc.text(`Staff promoted (in filter): ${report.numberOfStaff}`, 14, metaY);
    metaY += 6;
    if (pwdFilter !== 'all') {
      const pwdLabel =
        pwdFilter === 'yes'
  ...
  const exportExcel = () => {
    ...
    const ws = XLSX.utils.aoa_to_sheet([
      ...
      ['Staff promoted (in filter)', report.numberOfStaff],
      [],
      headerRow1,
      ...body,
    ]);
    const wb = XLSX.utils.book_new();
    XLSX.utils.book_append_sheet(wb, ws, 'Staff Promotions');
    XLSX.writeFile(wb, 'Staff_Promotions.xlsx');
  };
    ...
    <div className="table-card">
      ...
      {report && !loading && !error && (
        <div className="px-4 py-2 border-bottom bg-light small d-flex flex-wrap gap-3">
          ...
          <span>
            <span className="text-muted">Staff promoted:</span>{' '}
          ...
          <span className="text-muted">
            ...
            {report.rows.length === 1 ? '' : 's'} with promotions
          </span>
        </div>
      )}
            ...
            {loading ? (
              ...
              <tr>
                <td colSpan={colSpan} className="text-center py-4 text-muted">
                  No promotions match the selected filters for the current financial years.
                </td>
              </tr>
            ) : (
              <>
      ...
      {report && !loading && !error && (
        <div className="table-card-footer d-flex flex-wrap justify-content-between gap-2">
          <span className="footer-label">
            ...
            {report.numberOfStaff.toLocaleString()} staff promoted in current filter
          </span>
        </div>
      )}
    </div>
